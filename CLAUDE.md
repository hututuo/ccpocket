...
在本地兼容 fork 上调查、实现、合并、打包或发布前，必须完整阅读
[`docs/PROJECT_HANDOFF.md`](docs/PROJECT_HANDOFF.md)，再读取
`decisions.md` 中与任务相关的章节。该手册记录长期产品原则、用户要求、
官方更新兼容边界、验证门禁和多 Agent 协作规则；它不是当前版本状态表。

尤其注意：源码实现、Bridge 部署、OTA 发布、IPA 签名安装、真机验收和
stable 晋级是彼此独立的授权与完成状态。未经明确授权，不要合入共享分支、
push、发布、替换运行服务或安装应用。

## 云端接手入口

本项目的公开源码入口由 `.project-context.json` 声明；进入源码目录后，按以下顺序阅读：

1. `AGENTS.md`（本文件，规则与边界）；
2. `CONTEXT.md`、`PROJECT_INDEX.md`（公开的项目地图）；
3. `docs/PROJECT_HANDOFF.md`、`decisions.md`（长期原则与门禁）；
4. `.project-context.json` 中声明的私有开发资料项目（若当前身份有权限）。

私有资料项目只保存内部进度、排障证据、接手记录和验证结果，不作为源码或构建
依赖。源码提交、候选构建、签名/发布、运行时部署和真机验收必须分别记录，不得
把某一项的通过当成其他项已完成。若私有资料仓库不可读，必须明确标为
`UNKNOWN/BLOCKED`，不能用当前聊天补齐未读内容。
