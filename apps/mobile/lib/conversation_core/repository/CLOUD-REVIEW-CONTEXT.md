...
`_recoverProjectionInbox` replays pending rows in deterministic batches of 32.
An old complete projection must not delete a newer head. Projection-only thread
state is explicit and must not impersonate a verified canonical timeline.
...
- Capacity includes canonical, staging, projection, outbox, state, gap, and
  retired-evidence rows, not just visible items.
...
focused file plus affected analysis/format checks.
