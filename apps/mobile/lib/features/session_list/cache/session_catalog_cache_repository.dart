...
class SessionCatalogCacheRepository {
      ...
      await db.transaction((transaction) async {
        ...
        for (final entry in [...created, ...updated]) {
          final preservedAssistantOutputAt =
              await _cachedAssistantOutputAtForIdentity(
                transaction,
                partitionId: partitionId,
          ...
          final decodedSession = entry.toRecentSession(
            codexSourceId: codexSourceId,
          );
  ...
  static Future<String?> _cachedAssistantOutputAtForIdentity(
    DatabaseExecutor database, {
    required String partitionId,
    required String provider,
    required String providerSessionId,
    ...
    for (final row in rows) {
      try {
        ...
        latest = _latestIsoTimestamp(latest, candidate);
      } catch (_) {
