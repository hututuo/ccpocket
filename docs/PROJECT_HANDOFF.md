...
| 1.109.2 全面修复与性能收束主方案 | `plans/mobile-comprehensive-remediation_v01_20260725-012458.md` | 当前 active 执行清单；覆盖官方更新、会话/历史/折叠/临时会话、文件安全与预览、通知、未读、本地化、完整审计和性能优化 |
...
- 完成后主动向合并协调会话回报，不要求用户人工转发。

### 发布会话

- 复用累计的发布上下文，执行可审计的 Mobile/OTA/IPA 发布流程。
- 确认源码身份、基础版本、Flutter/原生边界、签名、owner/stable 前后状态和实际产物。
- 发现源码不干净、基础 lineage 不符、原生差异、未知 Shorebird 警告或上传状态不明确时必须停止并交回协调会话判断。
...
实现 Agent 完成后将回报发送给固定合并协调会话
`019f8ff9-0945-72a3-a29e-c17df6f112e5`，并保留分支和 worktree 供审核。
