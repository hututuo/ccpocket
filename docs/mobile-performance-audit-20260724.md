...
审计完成后的联网复核显示，官方 `upstream/main` 已从本地基线包含的
`0b793a157c04d24743360b90ae5aa8bc3f4e4abd`（`1.108.0+196`）前进到
`9ff2e76ce3e94af2b00c28b1192c6d08c5de781d`（`1.108.1+197`），中间还包含 Flutter
3.44.7 升级。两条官方提交只修改版本、依赖锁、分析规则、语言选择器和 mock 数据，与本性能分支
修改的源文件没有直接重叠。

为避免在多个并行 Agent 工作期间擅自重写共同基线，本分支保留在最新的本地集成功能基线
`169155ed`。协调合并时应先集成官方 `1.108.1+197`，再按顺序回放本分支提交，并使用 Flutter
3.44.7 重跑测试与 iOS 构建。
