...
| D07 | 当前通知/请求路由已适合多客户端共享 daemon。 | 当前普通通知按 threadId 过滤，但 server-initiated request 在进入 `handleServerRequest` 前没有同等线程过滤；不支持的方法会立即回 `-32601`。 | 多 attachment 时可能由错误 Bridge session 响应 Desktop 所属审批/问题，或多个 session 同时响应同一请求。 | Pilot 至少加入 thread-scoped request 过滤和单线程 attachment 唯一性；完整 Action Broker、持久请求台账与双客户端并发仲裁仍属于 Stage 3+。 | 两线程交错请求测试，非本 attachment 请求不得响应；同一 provider thread 只允许一个 pilot attachment。 |
