...
- Integrate official Bridge 1.69.5 image-heavy restore and generated-image
  thumbnail support while retaining the local compatibility stack.

### Fixed

- Discover Desktop/Cockpit-owned Codex Working state across the durable
  catalog, keep live status and current-turn content synchronized, and prevent
  completed external turns from being resurrected by a later catalog refresh.

### Performance

- Reuse one read-only app-server process, avoid idle rollout monitor seed
  parses, bound live content, and share rollout path indexes across discovery.
