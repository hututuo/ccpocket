...
import { BridgeDevicePairing, canonicalDeviceChallengePayload } from "./bridge-device-pairing.js";
...
describe("paired Bridge WebSocket admission", () => {
  ...
  it("sends only a challenge before authentication and activates legacy data after proof", async () => {
    ...
    const pairing = new BridgeDevicePairing({ stateDir: root, bridgeIdentity: identity, bridgeInstanceId: "bridge-instance" });
    await pairing.init();
    ...
    try {
      ...
      expect(messages.some((message) => message.type === "bridge_device_authenticated")).toBe(true);
      expect(messages.some((message) => message.type === "session_list")).toBe(true);
      expect(messages.some((message) => message.type === "project_history")).toBe(true);
    } finally {
      client.close();
      await bridge.close();
