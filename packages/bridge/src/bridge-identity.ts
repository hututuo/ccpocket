...
export function readBridgeComputerName(options: {
  env?: NodeJS.ProcessEnv;
  platform?: NodeJS.Platform;
  hostnameValue?: string;
  displayName?: string;
} = {}): string {
  const envName = options.displayName ?? options.env?.BRIDGE_DISPLAY_NAME ?? process.env.BRIDGE_DISPLAY_NAME;
  if (envName?.trim()) return normalizeBridgeDisplayName(envName);

  const platform = options.platform ?? process.platform;
  if (platform === "darwin") {
    try {
      const output = execFileSync("/usr/sbin/scutil", ["--get", "ComputerName"], {
        encoding: "utf8",
        timeout: 700,
...
export class BridgeIdentityStore {
  ...
  private constructor(
    ...
    publicKey: string,
    options: Required<Pick<BridgeIdentityStoreOptions, "stateDir" | "identityFile">> &
  ...
  static async load(options: BridgeIdentityStoreOptions = {}): Promise<BridgeIdentityStore> {
    const stateDir = options.stateDir ?? process.env.CCPOCKET_STATE_DIR ?? DEFAULT_STATE_DIR;
    const identityFile = options.identityFile ?? join(stateDir, DEFAULT_IDENTITY_FILE);
    await mkdir(stateDir, { recursive: true, mode: 0o700 });
    const stateStats = await lstat(stateDir);
    if (stateStats.isSymbolicLink() || !stateStats.isDirectory()) {
      throw new Error("Bridge identity state directory must be a real directory");
    }
    ...
    try {
      ...
      const value = JSON.parse(await readFile(identityFile, "utf8")) as Partial<BridgeIdentityFile>;
      if (
        value.version === 1 &&
      ...
      // A missing/corrupt identity is recovered by generating a fresh key.
    }
    if (parsed) await chmod(identityFile, 0o600).catch(() => undefined);
    ...
    if (parsed) {
      try {
        ...
        const persistedPublic = rawEd25519PublicKey(decodeBase64Url(parsed.publicKey));
        if (!rawDerived.equals(persistedPublic)) throw new Error("Bridge identity key mismatch");
      } catch {
        parsed = undefined;
      }
    }
    ...
    if (!parsed) {
      ...
      publicKey = base64url(rawEd25519PublicKey(generated.publicKey.export({ format: "der", type: "spki" })));
  ...
  response(input: {
    ...
    if (!isValidIdentityNonce(input.nonce)) {
      throw new Error("Identity nonce must be base64url with 16..96 characters");
    }
    ...
    return {
      ...
      methods: [...input.methods],
      signedPayload,
      signature: base64url(sign(null, Buffer.from(signedPayload, "utf8"), this.privateKey)),
    };
  }

  sign(payload: string | Buffer): string {
    return base64url(sign(null, Buffer.isBuffer(payload) ? payload : Buffer.from(payload, "utf8"), this.privateKey));
  }
