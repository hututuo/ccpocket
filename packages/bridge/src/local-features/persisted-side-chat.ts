...
export class PersistedSideChatFeatureHandler implements LocalFeatureHandler {
  ...
  async handle(
    ...
    if (!context.runtime.supports(context.client, RESPONSE_TYPE)) {
      this.sendFailure(message, context, "unsupported_capability", "Durable side chat was not negotiated");
      return;
    }
    const create = context.runtime.createPersistedCodexChildSession;
    if (!create) {
      this.sendFailure(message, context, "unsupported_bridge", "This Bridge cannot create a durable side chat");
      return;
    }
