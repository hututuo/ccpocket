...
describe("setup-systemd", () => {
  ...
  describe("setupSystemd", () => {
    ...
    it("persists file-transfer storage paths with systemd escaping", () => {
      ...
      const content = mockWriteFileSync.mock.calls[0]![1] as string;
      expect(content).toContain(
        'Environment="BRIDGE_FILE_TRANSFER_DOWNLOAD_DIR=/home/testuser/Phone \\"Files\\""',
      );
      expect(content).toContain("BRIDGE_FILE_TRANSFER_PARTIAL_DIR");
      expect(content).toContain("BRIDGE_FILE_TRANSFER_STATE_FILE");
    });
