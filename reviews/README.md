...
`fix/mobile-comprehensive-v02-20260726@95cae34b`；2026-07-28 重新 fetch 后的
`upstream/main` 为 `82962136`，其中导航修复已由 `c2cc8379` 结合本地嵌入式
工作区语义整合，版本记录由 `97fb5aab` 同步为 `1.109.3+205`。后续又完成
所选 `CODEX_HOME` 的 Bridge 读写统一、跨 provider 同 ID 保留和 Mobile 来源
缓存隔离；完整多 Home 注册表与跨进程单写者协议仍未完成。逐项状态和验证证据以
